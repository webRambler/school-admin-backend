...
public class CollegeServiceImpl implements ICollegeService {
    ...
    public CollegeServiceImpl(CollegeMapper collegeMapper, RedisService redisService) {
    ...
    public College createCollege(College college) {
        // 检查学院代码是否已存在
        College existing = collegeMapper.selectCollegeByCode(college.getCode());
        if (existing != null) {
    ...
    public College getCollegeById(Long id) {
        ...
        College college = collegeMapper.selectCollegeById(id);
        if (college == null) {
    ...
    public College getCollegeByCode(String code) {
        College college = collegeMapper.selectCollegeByCode(code);
        if (college == null) {
    ...
    public College updateCollege(Long id, College collegeDetails) {
        ...
        if (collegeDetails.getCode() != null) {
            // 检查新代码是否与其他学院冲突
            College existing = collegeMapper.selectCollegeByCode(collegeDetails.getCode());
    ...
    public void deleteCollege(Long id) {
        getCollegeById(id);
        // 检查学院下是否还有班级
        Long classCount = collegeMapper.countClassesByCollegeId(id);
        if (classCount > 0) {
